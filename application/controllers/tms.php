...
class Tms extends MY_Controller {
	...
	public function datetime_check($strDateTime,$field) {
		try {
			...
			if(substr($field, -3)=="End") {
				...
				if( $startDate >= $endDate ) {
					if($this->input->post($startDateField)) {
						$error = "$startDateField date: ".datetimestr_to_public($startDate)." is not before end date: ".datetimestr_to_public($endDate);
					} else {
						$error = "$endDateField date: ".datetimestr_to_public($startDate)." is in the past";
					}						
			...
			$this->form_validation->set_message('datetime_check', 'Exception: '.$e->getMessage().' DateTime string provided: '.var_dump($strDateTime,1) );
			return FALSE;
		}
	}
