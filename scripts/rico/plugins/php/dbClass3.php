...
/*****************************************************************
 Page        : dbClass3.php
 Description : Routines to access MySQL, PostgreSQL, SQL Server, Oracle, & ODBC databases
 Date        : 25 May 2006
 Author      : Matt Brown (dowdybrown@yahoo.com)
 Copyright (C) 2006-2011 Matt Brown
...
class sqlParse {
  ...
  function ToArray() {
    return array($this->arSelList,$this->arSelListAs,$this->FromClause,$this->WhereClause,$this->arGroupBy,$this->HavingClause,$this->arOrderBy,$this->IsDistinct);
  }

  function LoadArray($a) {
    $this->arSelList=$a[0];
    $this->arSelListAs=$a[1];
    $this->FromClause=$a[2];
    $this->WhereClause=$a[3];
    $this->arGroupBy=$a[4];
    $this->HavingClause=$a[5];
    $this->arOrderBy=$a[6];
    $this->IsDistinct=$a[7];
  }
  ...
  // -------------------------------------------------------------
  function Unparse($arSkipCols) {
    $sqltext="SELECT ";
    if ($this->IsDistinct) $sqltext.="DISTINCT ";
    $sqltext.=$this->UnparseColumnListSkip($arSkipCols)." FROM ".$this->FromClause;
  ...
  function UnparseSelect() {
    return $this->Unparse(array());
  }

  function UnparseSelectSkip($arSkipCols) {
    return $this->Unparse($arSkipCols);
  }
  ...
  function UnparseColumnListSkip($arSkipCols) {
    for ($i=0,$SkipIdx=0; $i<count($this->arSelList); $i++) {
      $skip=false;
      if ($SkipIdx < count($arSkipCols)) {
        $skip=($arSkipCols[$SkipIdx]==$i);
        if ($skip) $SkipIdx++;
      }
      if (!$skip) {
        if (empty($sqltext)) {
...
class dbClass_mysql
{
  ...
  function FieldName($rsMain,$i) { return mysql_field_name($rsMain,$i); }
...
class dbClass_oci
{
  ...
  function FieldName($rsMain,$i) { return oci_field_name($rsMain,$i); }
  ...
  function GetTableList($TableType) {
    if ($TableType!='VIEW'){
      $TableType='TABLE';
      $tableFiltre="and exists (select 'X' from USER_TABLES where TABLE_NAME = OBJECT_NAME) ";
    }
    $rsMain=$this->RunQuery("SELECT OBJECT_NAME FROM ALL_OBJECTS WHERE OBJECT_TYPE = '".$TableType."' and OWNER = UPPER('".$GLOBALS['appName']."') ".$tableFiltre."order by 1");
    if (!$rsMain) return null;
    $arTables=array();
    while($this->FetchRow($rsMain,$row)) {
      array_push($arTables, $row[0]);
...
class dbClass_odbc
{
  ...
  function FieldName($rsMain,$i) { return odbc_field_name($rsMain,$i); }
...
class dbClass_mssql
{
  ...
  function FieldName($rsMain,$i) { return mssql_field_name($rsMain,$i); }
  ...
  function GetColumnInfo($TableName) {
    ...
    while($this->FetchAssoc($rsMain,$row)) {
      if (strtoupper($row["TABLE_NAME"]) != $TableName) continue;  // sp_columns treats "Order_Details" and "Order Details" as the same
      $colinfo=new dbColumn;
      $colinfo->ColName=$row["COLUMN_NAME"];
      $colinfo->ColType=$row["TYPE_NAME"];
      if (array_key_exists("PRECISION",$row)) {
  ...
  function GetTableList($TableType) { 
    $typecode=$TableType!='VIEW' ? 'U' : 'V';
    $rsMain=$this->RunQuery("select name from sysobjects where type='".$typecode."' order by name"); 
    if (!$rsMain) return null; 
    $arTables=array(); 
    while($this->FetchRow($rsMain,$row)) { 
      array_push($arTables, $row[0]); 
...
//********************************************************************************************************
  function MySqlLogon($DefDB,$userid,$pw)
  {
    ...
    if ($this->CheckForError("opening connection to " . $this->Provider)) return false;
    mysql_select_db($DefDB,$this->dbMain);
    if ($this->CheckForError("selecting database " . $DefDB)) return false;
    return true;
  }
...
// Attempts to connect to the Database. Returns true on success.
//********************************************************************************************************
  function PostgreSqlLogon($connection_string)
  {
    $this->Dialect="PostgreSQL";
    $this->dbDefault = "";
    $this->dbMain = pg_connect($connection_string);
